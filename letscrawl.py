...
class CompanyCrawler:
    ...
    def get_company_details(self, bplc_id: str, year: str = "2022") -> Dict:
        ...
        try:
            ...
            if not table:
                table = soup.find('table', class_='tbl_st3')
            
            if not table:
                print(f"⚠️  업체 ID '{bplc_id}': 정보 테이블을 찾을 수 없습니다.")
                return {"bplcId": bplc_id, "error": "테이블을 찾을 수 없음"}
            ...
            print(f"✅ 업체 '{result.get('업체명', bplc_id)}' 정보 수집 완료")
            return result
